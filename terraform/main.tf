...
locals {
    name_prefix = "${var.app_name}-${var.environment}-${random_id_suffix.hex}"
}

data "aws_cannonical_user_id" "current" {}
...
resource "aws_s3_s3_bucket_ownership_controls" "logs" {
...
resource "aws_s3_bucket_acl" "logs" {
    bucket = aws_s3_bucket.logs.id
    depends_on = [aws_s3_s3_bucket_ownership_controls.logs]

    access_control_policy {
        owner {
            id = data.aws_cannonical_user_id.current.id
        }
